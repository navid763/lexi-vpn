...
export const approveTopupHandler = async (
    ctx: BotContext,
    adapter: BotAdapter
) => {
    ...
    if (action === "APPROVE_TOPUP") {
        try {
            // Run both the payment status update and balance increment atomically.
            ...
            await adapter.sendMessage(
                ...
                `💳 موجودی فعلی شما: ${(updatedCustomer.balance / 10).toLocaleString("fa-IR")} تومان`
            );
