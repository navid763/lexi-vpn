...
`
    ...
    try {
        ...
        const response = await approveOrderByWallet(
            order.toJSON().id,
            ctx.chatId,
            adapter
        );
        ...
        await adapter.sendMessage(ADMIN_CHAT_ID,
            `✅ سفارش تایید شد\n\nOrder ID: ${approvedOrder.id}\nUser: ${ctx.chatId}\nPayment: Wallet\nAmount: ${approvedOrder.price}`
        );

        await adapter.sendMessage(Number(ctx.chatId),
        ...
        await adapter.sendMessage(Number(ADMIN_CHAT_ID), `❌ خطا در سیستم حین تایید سفارش\n\nOrder: ${order.id}\nUser: ${ctx.chatId}`);
    }
